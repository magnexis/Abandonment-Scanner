...
export const getConfidenceLevel = (score, factors) => {
    const averageFactorConfidence = factors.reduce((total, factor) => total + factor.confidence, 0) / Math.max(factors.length, 1);
    if (score >= 70 && averageFactorConfidence >= 72)
        return "High";
    if (score >= 40 || averageFactorConfidence >= 64)
        return "Medium";
    return "Low";
};
export const calculateAbandonmentScore = (inputs) => {
    const factors = [
        {
            key: "last-sale",
